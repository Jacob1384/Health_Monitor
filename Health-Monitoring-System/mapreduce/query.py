...
# Query from date1 to date2. They should be in format (YYYY-MM-DD HH:MM:SS)
...
def query(date1, date2):
    ...
    for tup in years:
        ...
        final_df = pd.concat([final_df, sql('year.parquet', ts1, ts2)])
    # Querying months
    for tup in months:
        ts1 = tup[0] + '-01 00:00:00'
        ts2 = tup[1] + '-01 00:00:00'
        final_df = pd.concat([final_df, sql('month.parquet', ts1, ts2)])
    # Querying days
    for tup in days:
        ts1 = tup[0] + ' 00:00:00'
        ts2 = tup[1] + ' 00:00:00'
        final_df = pd.concat([final_df, sql('day.parquet', ts1, ts2)])
    # Querying hours
    for tup in hours:
        ts1 = tup[0] + ':00:00'
        ts2 = tup[1] + ':00:00'
        final_df = pd.concat([final_df, sql('hour.parquet', ts1, ts2)])
    # Querying minutes
    for tup in minutes:
        ts1 = tup[0] + ':00'
        ts2 = tup[1] + ':00'
        final_df = pd.concat([final_df, sql('minute.parquet', ts1, ts2)])
...
print(query('2022-1-1 12:56:22', '2025-2-12 23:34:12'))
